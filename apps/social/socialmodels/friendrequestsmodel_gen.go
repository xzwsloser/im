...
type (
	friendRequestsModel interface {
		...
		FindByReqIdAndUid(ctx context.Context , rid , uid string) (*FriendRequests , error)
...
func (m *defaultFriendRequestsModel) FindByReqIdAndUid(ctx context.Context , rid , uid string) (*FriendRequests , error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `req_uid` = ? ", friendRequestsRows, m.table)
	var resp FriendRequests
...
func (m *defaultFriendRequestsModel) ListNoHandler(ctx context.Context , uid string) ([]*FriendRequests , error) {
